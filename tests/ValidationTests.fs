...
type ValidationTestFixture() =
    ...
    let validatedUsers = db.Collection<ValidatedUser>("validated_users", validatedUserSchema)
    let noValidationUsers = db.Collection<ValidatedUser>("no_validation_users", noValidationSchema)
...
type ValidationTests(fixture: ValidationTestFixture) =
    ...
    member _.``insertOne with invalid email returns Validation error`` () : Task =
        task {
            ...
            | Ok _ ->
                failwith "Expected Validation error, got Ok"
            ...
            | Error err ->
                failwith $"Expected Validation error, got different error: {err}"
        }
    ...
    member _.``schema without validator accepts any data`` () : Task =
        task {
            ...
            | Error err ->
                failwith $"Expected Ok (no validation), got Error: {err}"
        }
