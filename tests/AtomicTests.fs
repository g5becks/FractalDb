...
let taskSchema : SchemaDef<TodoTask> = {
    Fields = [
        ...
        { Name = "status"; Path = None; SqlType = SqliteType.Text; Indexed = true; Unique = false; Nullable = false }
        { Name = "priority"; Path = None; SqlType = SqliteType.Integer; Indexed = true; Unique = false; Nullable = false }
    ]
    Indexes = []
    Timestamps = true
    Validate = None
...
type AtomicTests(fixture: AtomicTestFixture) =
    ...
    [<Fact>]
    member _.``findOneAndDelete returns deleted document`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_findAndDelete", taskSchema)
        
        let tasksToInsert = [
            { Name = "Task1"; Status = "pending"; Priority = 1 }
            { Name = "Task2"; Status = "pending"; Priority = 2 }
            { Name = "Task3"; Status = "completed"; Priority = 3 }
        ]
        
        let! _ = tasks |> Collection.insertMany tasksToInsert
        
        // Act - Delete first pending task
        let query = Query.field "status" (Query.eq "pending")
    ...
    [<Fact>]
    member _.``findOneAndDelete returns None if not found`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_deleteNotFound", taskSchema)
        
        let tasksToInsert = [
            { Name = "Task1"; Status = "completed"; Priority = 1 }
            { Name = "Task2"; Status = "completed"; Priority = 2 }
        ]
    ...
    [<Fact>]
    member _.``findOneAndUpdate with ReturnDocument.Before returns original`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_updateBefore", taskSchema)
        
        let initialTask = { Name = "UpdateTest"; Status = "pending"; Priority = 5 }
        let! insertResult = tasks |> Collection.insertOne initialTask
        let insertedDoc = match insertResult with Ok doc -> doc | Error e -> failwith e.Message
        ...
        let query = Query.field "name" (Query.eq "UpdateTest")
        let! result = 
            tasks |> Collection.findOneAndUpdate 
                query 
                (fun t -> { t with Status = "in-progress"; Priority = 10 })
                options
        ...
        | Ok None ->
            failwith "Expected Some document, got None"
        | Error err ->
            failwith $"Expected Ok, got Error: {err.Message}"
    }

    [<Fact>]
    member _.``findOneAndUpdate with ReturnDocument.After returns modified`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_updateAfter", taskSchema)
        
        let initialTask = { Name = "UpdateTest2"; Status = "pending"; Priority = 5 }
        let! insertResult = tasks |> Collection.insertOne initialTask
        let insertedDoc = match insertResult with Ok doc -> doc | Error e -> failwith e.Message
        ...
        let query = Query.field "name" (Query.eq "UpdateTest2")
        let! result = 
            tasks |> Collection.findOneAndUpdate 
                query 
                (fun t -> { t with Status = "completed"; Priority = 20 })
                options
    ...
    [<Fact>]
    member _.``findOneAndUpdate with sort selects correct document`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_updateSort", taskSchema)
        
        let tasksToInsert = [
            { Name = "Task1"; Status = "pending"; Priority = 3 }
            { Name = "Task2"; Status = "pending"; Priority = 1 }  // Lowest priority
            { Name = "Task3"; Status = "pending"; Priority = 5 }
        ]
        ...
        let! result = 
            tasks |> Collection.findOneAndUpdate 
                query 
                (fun t -> { t with Status = "in-progress" })
                options
    ...
    [<Fact>]
    member _.``findOneAndReplace replaces document body`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_replace", taskSchema)
        
        let originalTask = { Name = "OriginalTask"; Status = "pending"; Priority = 5 }
        let! insertResult = tasks |> Collection.insertOne originalTask
        let insertedDoc = match insertResult with Ok doc -> doc | Error e -> failwith e.Message
        
        // Act - Replace entire document
        let replacementTask = { Name = "ReplacedTask"; Status = "completed"; Priority = 10 }
        ...
        let query = Query.field "name" (Query.eq "OriginalTask")
        let! result = 
            tasks |> Collection.findOneAndReplace 
                query 
                replacementTask
                options
    ...
    [<Fact>]
    member _.``findOneAndReplace with Before returns original`` () : Task = task {
        // Arrange
        let db = fixture.Db
        let tasks = db.Collection<TodoTask>("tasks_replaceBefore", taskSchema)
        
        let originalTask = { Name = "BeforeTest"; Status = "pending"; Priority = 3 }
        let! insertResult = tasks |> Collection.insertOne originalTask
        let insertedDoc = match insertResult with Ok doc -> doc | Error e -> failwith e.Message
        
        // Act - Replace and return Before state
        let replacementTask = { Name = "AfterTest"; Status = "completed"; Priority = 7 }
        ...
        let query = Query.field "name" (Query.eq "BeforeTest")
        let! result = 
            tasks |> Collection.findOneAndReplace 
                query 
                replacementTask
    ...
    [<Fact>]
    member _.``atomic operations are truly atomic`` () : Task = task {
        // Arrange
        let db = fixture.Db
        ...
        | Ok (Some doc) ->
            ...
            let pendingQuery = Query.field "status" (Query.eq "pending")
