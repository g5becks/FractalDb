...
let productSchema : SchemaDef<Product> = {
    Fields = [
        ...
        { Name = "price"; Path = None; SqlType = SqliteType.Integer; Indexed = true; Unique = false; Nullable = false }
        { Name = "inStock"; Path = None; SqlType = SqliteType.Integer; Indexed = true; Unique = false; Nullable = false }
    ]
    Indexes = []
    Timestamps = true
    Validate = None
...
type QueryTestFixture() =
    ...
    do
        let sampleProducts = [
            { Name = "Laptop"; Category = "Electronics"; Price = 1200; InStock = true; Tags = ["tech"; "portable"] }
            { Name = "Mouse"; Category = "Electronics"; Price = 25; InStock = true; Tags = ["tech"; "accessory"] }
            { Name = "Keyboard"; Category = "Electronics"; Price = 75; InStock = false; Tags = ["tech"; "accessory"] }
            { Name = "Desk"; Category = "Furniture"; Price = 300; InStock = true; Tags = ["office"; "large"] }
            { Name = "Chair"; Category = "Furniture"; Price = 150; InStock = true; Tags = ["office"; "comfort"] }
            { Name = "Monitor"; Category = "Electronics"; Price = 400; InStock = true; Tags = ["tech"; "display"] }
            { Name = "Lamp"; Category = "Furniture"; Price = 50; InStock = false; Tags = ["office"; "light"] }
            { Name = "Tablet"; Category = "Electronics"; Price = 600; InStock = true; Tags = ["tech"; "portable"] }
            { Name = "Bookshelf"; Category = "Furniture"; Price = 200; InStock = true; Tags = ["office"; "storage"] }
            { Name = "Webcam"; Category = "Electronics"; Price = 100; InStock = false; Tags = ["tech"; "accessory"] }
        ]
...
type QueryExecutionTests(fixture: QueryTestFixture) =
    ...
    [<Fact>]
    member _.``Query.eq filters correctly`` () : Task = task {
    ...
    [<Fact>]
    member _.``Query.gte and Query.lt range query works`` () : Task = task {
    ...
    [<Fact>]
    member _.``Query.contains string operator works`` () : Task = task {
        // Query products with name containing "top"
        let query = Query.field "name" (Query.contains "top")
        let! results = products |> Collection.find query
    ...
    [<Fact>]
    member _.``Query.isIn filters by list`` () : Task = task {
        // Query products in Furniture or with specific names
        let query = Query.field "category" (Query.isIn ["Furniture"])
        let! results = products |> Collection.find query
    ...
    [<Fact>]
    member _.``Query.all' combines conditions with AND`` () : Task = task {
        // Query Electronics products that are in stock
        let query = Query.all' [
            Query.field "category" (Query.eq "Electronics")
            Query.field "inStock" (Query.eq true)
        ]
        let! results = products |> Collection.find query
    ...
    [<Fact>]
    member _.``Query.any combines conditions with OR`` () : Task = task {
        // Query products that are either very cheap (<= 50) OR very expensive (>= 1000)
        let query = Query.any [
            Query.field "price" (Query.lte 50)
            Query.field "price" (Query.gte 1000)
        ]
        let! results = products |> Collection.find query
    ...
    [<Fact>]
    member _.``QueryOptions sort orders correctly`` () : Task = task {
        // Query all products sorted by price ascending
        let query = Query.empty<Product>
    ...
    [<Fact>]
    member _.``QueryOptions limit restricts results`` () : Task = task {
    ...
    [<Fact>]
    member _.``QueryOptions skip and limit for pagination`` () : Task = task {
    ...
    [<Fact>]
    member _.``Complex query with multiple conditions and sorting`` () : Task = task {
