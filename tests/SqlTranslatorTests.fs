...
let ``Multiple fields with same value use separate parameters`` () =
    ...
    result.Parameters
    |> List.iter (fun (_, value) -> (unbox<string> value) |> should equal "test")
...
let ``Nor with single query`` () =
    ...
    let query =
        Query.Nor [ Query.Field("name", FieldOp.Compare(box (CompareOp.Eq "Alice"))) ]
...
let ``Null value in In operator`` () =
    ...
    let query =
        Query.Field("status", FieldOp.Compare(box (CompareOp.In [ "active"; null; "pending" ])))
...
let ``Very long And list generates correct SQL`` () =
    ...
    let andCount =
        result.Sql.Split([| " AND " |], System.StringSplitOptions.None).Length - 1
