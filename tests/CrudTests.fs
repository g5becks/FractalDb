...
type CrudTests(fixture: CrudTestFixture) =
    ...
    member _.``insertOne fails with UniqueConstraint for duplicate email``() : Task =
        task {
            ...
            match result with
            | Error(FractalError.UniqueConstraint(field, _)) -> field.ToLower() |> should equal "email"
            | Error err -> failwith $"Expected UniqueConstraint, got different error: {err.Message}"
            ...
            let queryFilter =
                Query.And
                    [ Query.Field("active", FieldOp.Compare(box (CompareOp.Eq true)))
                      Query.Field("name", FieldOp.String(StringOp.StartsWith $"FindTest{testPrefix}")) ]
    ...
    [<Fact>]
    member _.``updateById modifies document and updates timestamp``() : Task =
        task {
            let! insertResult =
                users
                |> Collection.insertOne
            ...
            | Ok doc ->
                ...
                let! updateResult = users |> Collection.updateById doc.Id (fun u -> { u with Age = 36 })
