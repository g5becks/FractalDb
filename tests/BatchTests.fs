...
type BatchTests(fixture: BatchTestFixture) =
    ...
    [<Fact>]
    member _.``insertMany inserts all documents``() : Task =
        task {
            // Arrange
            let db = fixture.Db
    ...
    [<Fact>]
    member _.``insertMany returns correct InsertedCount``() : Task =
        task {
            // Arrange
            let db = fixture.Db
            ...
            | Error err, _
            | _, Error err -> failwith $"Expected Ok, got Error: {err.Message}"
        }

    [<Fact>]
    member _.``insertMany handles empty list``() : Task =
        task {
            // Arrange
            let db = fixture.Db
    ...
    [<Fact>]
    member _.``updateMany updates all matching documents``() : Task =
        task {
            // Arrange
            let db = fixture.Db
            ...
            | Ok updateResult ->
                ...
                let furnitureQuery =
                    Query.Field("category", FieldOp.Compare(box (CompareOp.Eq "Furniture")))
    ...
    [<Fact>]
    member _.``updateMany with price increase``() : Task =
        task {
            // Arrange
            let db = fixture.Db
            ...
            let query = Query.Field("price", FieldOp.Compare(box (CompareOp.Lt 50)))

            let! result =
                products
                |> Collection.updateMany query (fun p -> { p with Price = p.Price * 3 / 2 })
    ...
    [<Fact>]
    member _.``deleteMany removes all matching documents``() : Task =
        task {
            // Arrange
            let db = fixture.Db
            ...
            let query = Query.Field("inStock", FieldOp.Compare(box (CompareOp.Eq false)))
    ...
    [<Fact>]
    member _.``deleteMany with complex query``() : Task =
        task {
            // Arrange
            let db = fixture.Db
            ...
            let query =
                Query.And
                    [ Query.Field("category", FieldOp.Compare(box (CompareOp.Eq "Electronics")))
                      Query.Field("price", FieldOp.Compare(box (CompareOp.Lt 50))) ]
